...
#define USB_TRANSFER_SIZE_BITBANG  384 // in multiple of 64 bytes (default 4096)
#define DATA_TRANSFER_SIZE_MPSSE   (USB_TRANSFER_SIZE_MPSSE*62/64)
#define DATA_TRANSFER_SIZE_BITBANG (USB_TRANSFER_SIZE_BITBANG*62/64)
#define USB_LATENCY_TIMER_MPSSE    2 // in ms (default 16)
#define USB_LATENCY_TIMER_BITBANG  4 // in ms (default 16)
...
void mexFunction (int nlhs, mxArray * plhs[], int nrhs, const mxArray * prhs[])
{
	...
	else if (strcmp(CommandName, "SPI2") == 0)
	{
		...
		// Divide into burst of DATA_TRANSFER_SIZE_BITBANG bytes
		NumBytesToSend = 0;
		NumBytesRead1 = 0;
		for (i = 0; i < NumBytesToSend1; i++)
		{
			OutputBuffer[NumBytesToSend++] = OutputBuffer1[i];
			if ((i == (NumBytesToSend1-1)) || ((NumBytesToSend % DATA_TRANSFER_SIZE_BITBANG) == 0))
			{
				// USB transaction
