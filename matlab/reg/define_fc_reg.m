...
reg(n).read_only = 1;
reg(n).flash = 1;
reg(n).subf{1} = {'VERSION',7,0,'uint8',41};
...
reg(n).subf{2} = {'VERSION_MINOR',15,8,'uint8',0};
...
reg(n).subf{4} = {'RADIO_HOST_CTRL',3,3,'uint8',0};
...
n = n + 1;
reg(n).name = 'MOTOR_PERIOD';
reg(n).read_only = 0;
reg(n).flash = 1;
reg(n).subf{1} = {'MOTOR_PERIOD',8,1,'uint8',1};
...
reg(n).read_only = 0;
reg(n).flash = 1;
reg(n).subf{1} = {'VBAT_SCALE',31,0,'single',0.0098};
