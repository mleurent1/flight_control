classdef mpu_reg
	methods
		function data = read(obj,addr)
			global ser
			fwrite(ser,[2,2,2,128+addr,0]);
			r = fread(ser,2);
			data = r(2);
		end
		function write(obj,addr,data)
			global ser
			fwrite(ser,[2,length(data)+1,length(data)+1,addr,data]);
		...
		function y = INT_PIN_CFG__I2C_BYPASS(obj,x)
			r = obj.read(55);
			if nargin < 2
		...
		function y = INT_PIN_CFG__FSYNC_INT_EN(obj,x)
			r = obj.read(55);
			if nargin < 2
		...
		function y = INT_PIN_CFG__FSYNC_INT_LVL(obj,x)
			r = obj.read(55);
			if nargin < 2
		...
		function y = INT_PIN_CFG__INT_RD_CLR(obj,x)
			r = obj.read(55);
			if nargin < 2
		...
		function y = INT_PIN_CFG__LATCH_INT_EN(obj,x)
			r = obj.read(55);
			if nargin < 2
				y = bitshift(bitand(r, 32), -5);
			else
		...
		function y = INT_PIN_CFG__INT_OPEN(obj,x)
			r = obj.read(55);
			if nargin < 2
				y = bitshift(bitand(r, 64), -6);
			else
		...
		function y = INT_PIN_CFG__INT_LVL(obj,x)
			r = obj.read(55);
			if nargin < 2
				y = bitshift(bitand(r, 128), -7);
			else
		...
		function y = INT_EN__I2C_MST_EN(obj,x)
			r = obj.read(56);
			if nargin < 2
				y = bitshift(bitand(r, 8), -3);
			else
		...
		function y = INT_STATUS__DATA_RDY_INT(obj,x)
			r = obj.read(58);
			if nargin < 2
				y = bitshift(bitand(r, 1), 0);
			else
		...
		function y = INT_STATUS__I2C_MST_INT(obj,x)
			r = obj.read(58);
			if nargin < 2
				y = bitshift(bitand(r, 8), -3);
			else
		...
		function y = INT_STATUS__FIFO_OFLOW_INT(obj,x)
			r = obj.read(58);
			if nargin < 2
				y = bitshift(bitand(r, 16), -4);
			else
