...
#define FLAG_REG 0x08
#define FLAG_SPI_RETURN_ON_USB 0x10
...
uint8_t spi2_rx_buffer[16];
uint8_t spi2_tx_buffer[16];
...
uint32_t motor1_dshot[17];
uint32_t motor2_dshot[17];
uint32_t motor3_dshot[17];
uint32_t motor4_dshot[17];
...
void SetDmaTim()
{
	...
	DMA1_Channel7->CCR = DMA_CCR_MINC | DMA_CCR_DIR | DMA_CCR_PL_1 | DMA_CCR_PSIZE_1 | DMA_CCR_MSIZE_1;
...
{
	while (((FLAG & FLAG_SENSOR) == 0) && (TIM15->CNT < TIMEOUT_SENSOR))
		__WFI();
}
...
void dshot_encode(uint32_t* val, uint32_t buf[17])
{
	int i;
	uint8_t bit[11];
	for (i=0; i<11; i++)
...
void DMA1_Channel4_IRQHandler()
{
	...
	else if (REG_CTRL__MPU_HOST_CTRL == 0)
	{
		...
		{
			REG_TIME &= ~0xFFFF;
			REG_TIME |= (uint32_t)time_spi & 0xFF;
		}
		#ifdef DEBUG
			GPIOA->BSRR = GPIO_BSRR_BR_4;
		#endif
	}
	else if (FLAG & FLAG_SPI_RETURN_ON_USB)
	{
...
int main()
{
	...
	vbat_acc = REG_VBAT_MIN / VBAT_ALPHA;
	vbat = REG_VBAT_MIN;
	vbat_sample_count = 0;
	...
	// Timer for sensor to motor processing time
	TIM7->PSC = 71; // 1us
	TIM7->ARR = 65535;
	TIM7->CR1 = TIM_CR1_CEN;
	
	// Timer for SPI transaction time
	TIM15->PSC = 71; // 1us
	TIM15->ARR = 65535;
	...
	NVIC_SetPriority(USB_LP_CAN_RX0_IRQn,3);
	...
	SpiWrite(MPU_PWR_MGMT_1, MPU_PWR_MGMT_1__CLKSEL(3));// | MPU_PWR_MGMT_1__TEMP_DIS); // Get MPU out of sleep, set CLK = gyro Z clock  and disable temperature sensors
	wait_ms(100);
	...
	//SpiWrite(MPU_INT_PIN_CFG, MPU_INT_PIN_CFG__INT_RD_CLR); // Clear IT on any read
	//wait_ms(100); // wait for filter to settle
	SpiWrite(MPU_INT_EN, MPU_INT_EN__DATA_RDY_EN);
	
	spi2_tx_buffer[1] = 0;
	SPI2->CR1 &= ~SPI_CR1_BR; // SPI clock = clock APB1/4 = 36MHz/4 = 9MHz
		...
		if (FLAG & FLAG_DEBUG)
		{
			...
			switch (REG_DEBUG)
			{
				case 1:
				{
					...
					USBD_CDC_Itf_tx_buffer[ 1] = (uint8_t)(sensor_sample_count >> 8);
					USBD_CDC_Itf_tx_buffer[ 2] = (uint8_t) gyro_x_raw;
					USBD_CDC_Itf_tx_buffer[ 3] = (uint8_t)(gyro_x_raw >> 8);
					USBD_CDC_Itf_tx_buffer[ 4] = (uint8_t) gyro_y_raw;
					USBD_CDC_Itf_tx_buffer[ 5] = (uint8_t)(gyro_y_raw >> 8);
					USBD_CDC_Itf_tx_buffer[ 6] = (uint8_t) gyro_z_raw;
					USBD_CDC_Itf_tx_buffer[ 7] = (uint8_t)(gyro_z_raw >> 8);
					USBD_CDC_Itf_tx_buffer[ 8] = (uint8_t) accel_x_raw;
					USBD_CDC_Itf_tx_buffer[ 9] = (uint8_t)(accel_x_raw >> 8);
					USBD_CDC_Itf_tx_buffer[10] = (uint8_t) accel_y_raw;
					USBD_CDC_Itf_tx_buffer[11] = (uint8_t)(accel_y_raw >> 8);
					USBD_CDC_Itf_tx_buffer[12] = (uint8_t) accel_z_raw;
					USBD_CDC_Itf_tx_buffer[13] = (uint8_t)(accel_z_raw >> 8);
					USBD_CDC_Itf_tx_buffer[14] = (uint8_t) temperature_raw;
					USBD_CDC_Itf_tx_buffer[15] = (uint8_t)(temperature_raw >> 8);
					USBD_CDC_SetTxBuffer(&USBD_device_handler, USBD_CDC_Itf_tx_buffer, 16);
					USBD_CDC_TransmitPacket(&USBD_device_handler);
					break;
				}
				case 2:
				{
					USBD_CDC_Itf_tx_buffer[0] = (uint8_t) sensor_sample_count;
					USBD_CDC_Itf_tx_buffer[1] = (uint8_t)(sensor_sample_count >> 8);
					float_to_bytes(&gyro_x, &USBD_CDC_Itf_tx_buffer[2]);
					float_to_bytes(&gyro_y, &USBD_CDC_Itf_tx_buffer[6]);
					float_to_bytes(&gyro_z, &USBD_CDC_Itf_tx_buffer[10]);
					float_to_bytes(&accel_x, &USBD_CDC_Itf_tx_buffer[14]);
					float_to_bytes(&accel_y, &USBD_CDC_Itf_tx_buffer[18]);
					float_to_bytes(&accel_z, &USBD_CDC_Itf_tx_buffer[22]);
					float_to_bytes(&temperature, &USBD_CDC_Itf_tx_buffer[26]);
					USBD_CDC_SetTxBuffer(&USBD_device_handler, USBD_CDC_Itf_tx_buffer, 30);
					USBD_CDC_TransmitPacket(&USBD_device_handler);
					break;
				}
				case 3:
				{
					USBD_CDC_Itf_tx_buffer[0] = (uint8_t) vbat_sample_count;
					USBD_CDC_Itf_tx_buffer[1] = (uint8_t)(vbat_sample_count >> 8);
					float_to_bytes(&vbat, &USBD_CDC_Itf_tx_buffer[2]);
					USBD_CDC_SetTxBuffer(&USBD_device_handler, USBD_CDC_Itf_tx_buffer, 6);
					USBD_CDC_TransmitPacket(&USBD_device_handler);
					break;
				}
				case 4:
				{
					...
					USBD_CDC_Itf_tx_buffer[ 1] = (uint8_t)(command_frame_count >> 8);
					USBD_CDC_Itf_tx_buffer[ 2] = (uint8_t) throttle_raw;
					USBD_CDC_Itf_tx_buffer[ 3] = (uint8_t)(throttle_raw >> 8);
					USBD_CDC_Itf_tx_buffer[ 4] = (uint8_t) aileron_raw;
					USBD_CDC_Itf_tx_buffer[ 5] = (uint8_t)(aileron_raw>> 8);
					USBD_CDC_Itf_tx_buffer[ 6] = (uint8_t) elevator_raw;
					USBD_CDC_Itf_tx_buffer[ 7] = (uint8_t)(elevator_raw>> 8);
					USBD_CDC_Itf_tx_buffer[ 8] = (uint8_t) rudder_raw;
					USBD_CDC_Itf_tx_buffer[ 9] = (uint8_t)(rudder_raw >> 8);
					USBD_CDC_Itf_tx_buffer[10] = (uint8_t) armed_raw;
					USBD_CDC_Itf_tx_buffer[11] = (uint8_t)(armed_raw >> 8);
					USBD_CDC_Itf_tx_buffer[12] = (uint8_t) chan6_raw;
					USBD_CDC_Itf_tx_buffer[13] = (uint8_t)(chan6_raw >> 8);
					USBD_CDC_SetTxBuffer(&USBD_device_handler, USBD_CDC_Itf_tx_buffer, 14);
					USBD_CDC_TransmitPacket(&USBD_device_handler);
					break;
				}
				case 5:
				{
					USBD_CDC_Itf_tx_buffer[ 0] = (uint8_t) command_frame_count;
					USBD_CDC_Itf_tx_buffer[ 1] = (uint8_t)(command_frame_count >> 8);
					float_to_bytes(&throttle, &USBD_CDC_Itf_tx_buffer[2]);
					float_to_bytes(&aileron, &USBD_CDC_Itf_tx_buffer[6]);
					float_to_bytes(&elevator, &USBD_CDC_Itf_tx_buffer[10]);
					float_to_bytes(&rudder, &USBD_CDC_Itf_tx_buffer[14]);
					USBD_CDC_SetTxBuffer(&USBD_device_handler, USBD_CDC_Itf_tx_buffer, 18);
					USBD_CDC_TransmitPacket(&USBD_device_handler);
					break;
				}
				case 6:
				{
					USBD_CDC_Itf_tx_buffer[0] = (uint8_t) sensor_sample_count;
					USBD_CDC_Itf_tx_buffer[1] = (uint8_t)(sensor_sample_count >> 8);
					float_to_bytes(&pitch, &USBD_CDC_Itf_tx_buffer[2]);
					float_to_bytes(&roll, &USBD_CDC_Itf_tx_buffer[6]);
					float_to_bytes(&yaw, &USBD_CDC_Itf_tx_buffer[10]);
					USBD_CDC_SetTxBuffer(&USBD_device_handler, USBD_CDC_Itf_tx_buffer, 14);
					USBD_CDC_TransmitPacket(&USBD_device_handler);
					break;
				}
				case 7:
				{
					...
					USBD_CDC_Itf_tx_buffer[1] = (uint8_t)(sensor_sample_count >> 8);
					for (i=0; i<4; i++)
						float_to_bytes(&motor[i], &USBD_CDC_Itf_tx_buffer[i*4+2]);
					USBD_CDC_SetTxBuffer(&USBD_device_handler, USBD_CDC_Itf_tx_buffer, 18);
					USBD_CDC_TransmitPacket(&USBD_device_handler);
					break;
				}
				case 8:
				{
					...
					for (i=0; i<4; i++)
					{
						USBD_CDC_Itf_tx_buffer[i*2+2] = (uint8_t) motor_raw[i];
						USBD_CDC_Itf_tx_buffer[i*2+3] = (uint8_t)(motor_raw[i] >> 8);
					}
					USBD_CDC_SetTxBuffer(&USBD_device_handler, USBD_CDC_Itf_tx_buffer, 10);
					USBD_CDC_TransmitPacket(&USBD_device_handler);
					break;
				}
			}
